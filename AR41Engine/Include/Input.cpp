...
void CInput::UpdateMouse(float deltaTime)
{
	...
	m_mouseImg->SetPos(m_MouseWorldPos);
	m_mouseWindow->Update(deltaTime);
// 
	//이렇게 하면 input의 cameracomponent 지워줘야 함
...
void CInput::ComputeWorldMousePos(const Vector2& cameraPos)
{
	...
	if (m_MouseWorldPos.x > 0.f && m_MouseWorldPos.x < (float)RS.width &&
		m_MouseWorldPos.y>0.f && m_MouseWorldPos.y < (float)RS.height)
	{
		ShowCursor(FALSE);
